window.onload = () => {
	...
	$RiddleText.textContent = "Guess the movie! (in polish)";
	...
	$LetterInput.setAttribute("onkeyup", "this.value = this.value.toUpperCase()");
	...
	$Submit.addEventListener("click", letter => {
		if (arrWW.includes($LetterInput.value)) {
			for (i = 0; i < arrWW.length; i++)
				if (
					arrWW[i] === $LetterInput.value &&
					arrWW[i] != arrWWNode[i].value &&
					$LetterInput.value != " " &&
					hangmanProgress != endDraw
				) {
